...
/**
 * wrapper on top of [DirectoryWatcher] that supports watching individual files as well as directories
 */
internal abstract class FileWatcher(vararg paths: Path) {
    ...
    /**
     * the paths to watch that we actually care about. for example if we are watching a single file we need to specify
     * its parent directory to the [watcher], then filter out events related to files we don't care about
     */
    val paths: MutableSet<Path> = paths.toMutableSet()

    /**
     * whether the watcher is watching a directory normally using [DirectoryWatcher] (`false`) or watching individual
     * files (`true`)
     */
    ...
    init {
        ...
        isWatchingFiles = if (this.paths.count() == 1) {
        ...
        this.watcher = DirectoryWatcher.builder()
            ...
            .listener { event ->
                if (
                    !isWatchingFiles
                    // since DirectoryWatcher doesn't support watching an individual files we watch the whole directory instead
